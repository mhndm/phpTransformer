...
function NewPool() {
    ...
    if (isset($_POST['SaveNewPool'])) {
        ...
        if (isset($_POST['poolstart'])) {
            $poolstart = PostFilter($_POST['poolstart']);
        } else {
            $poolstart = '';
        }
        if (isset($_POST['poolend'])) {
                                ...
                                <script language="javascript" type="text/javascript">
                                    document.onkeydown = document.onkeypress = function (evt) {
                                        ...
                                        if (keyCode) {
                                            if (evt.ctrlKey) {
                                                if(keyCode==83){
                                                                        document.getElementById("SaveNewPool").click();
                                                    return false;
                                                }
                                                return false;
                                            }
        ...
        $NewPool = '<script src="Blocks/Pool/SpryValidationTextField.js" type="text/javascript"></script>
					...
					' . PoolInterval . ' , ' . (From) . ' : 
					  ...
					  <input class="text" style="width:150px;" type="text" name="poolstart" id="poolstart" />
					  <span class="textfieldRequiredMsg">' . Avalueisrequired . '</span></span>' . (To) . ' : 
					  <span id="sprytextfield2">
					  <input class="text" style="width:150px;" type="text" name="poolend" id="poolend" />
					  <span class="textfieldRequiredMsg">' . Avalueisrequired . '</span></span>
                                          <br/><input onclick="NoStartOrEndDate();" name="NoDateInterval" 
                                          id="NoDateInterval" type="checkbox"  /><label for="NoDateInterval">' . NoDateInterval . '</label>
        ...
        $NewPool .= '</tr>';
        for ($k = 0; $k < 4; $k++) {
            $NewPool .= '<td><label  for="' . $k . '">' . Choise . ' ' . ($k + 1) . ' </label></td>
					    <td align="center">
					      <input type="radio" name="cheked" id="' . $k . '" value="' . $k . '"></td>';
            $query = " select * from `languages` where `Deleted`!='1'; ";
            ExcuteQuery($query);
            if ($TotalRecords > 0) {
        ...
        $NewPool .= '</table>
					<input class="submit" name="SaveNewPool" id="SaveNewPool" type="submit" value="' . (save) . '">
						</form>
        ...
        $NewPool .= '//-->\n
					...
					onUpdate       :    catcalc
                            ...
                            function NoStartOrEndDate(){
                                if(document.forms["formNewPool"].NoDateInterval.checked){
                                    document.forms["formNewPool"].poolstart.value="0000-00-00 00:00:00";
                                    document.forms["formNewPool"].poolend.value="0000-00-00 00:00:00";
                                    document.forms["formNewPool"].poolstart.disabled =true;
                                    document.forms["formNewPool"].poolend.disabled =true;
                                }
                                else{
                                    document.forms["formNewPool"].poolstart.value="";
                                    document.forms["formNewPool"].poolend.value="";
                                    document.forms["formNewPool"].poolstart.disabled =false;
                                    document.forms["formNewPool"].poolend.disabled =false;
                                }
...
function EditPool() {
    ...
    if (isset($_POST['SaveEditPool'])) {
        ...
        for ($j = 0; $j <= $NumberOfChoises; $j++) {
            ...
            //echo $idpc ;
            if ($cheked == $j) {
                mysqli_query($conn, "UPDATE `poolchoices` SET `cheked`='1'
				WHERE `idpc`='" . $idpc . "' AND `Idpt`='" . $Idpt . "';");
            } else {
                mysqli_query($conn, "UPDATE `poolchoices` SET `cheked`='0'
				WHERE `idpc`='" . $idpc . "' AND `Idpt`='" . $Idpt . "';");
            }
        }//end for
        ...
        ExcuteQuery($query);
        if ($TotalRecords > 0) {
            mysqli_query($conn, " delete from `poollangtitles` where `Idpt`='" . $Idpt . "' ;");
            mysqli_query($conn, " delete from `poollangtitles` where `Idpt`='" . $Idpt . "' ;");
            for ($i = 0; $i < $TotalRecords; $i++) {
                $LangName = $Rows['LangName'];
                $IdLang = $Rows['IdLang'];
                ...
                mysqli_query($conn, "insert into `poollangtitles` (`Title`,`IdLang`,`Idpt`) values('" . $Title . "','" . $IdLang . "','" . $Idpt . "' ) ; ");
                for ($k = 0; $k <= $NumberOfChoises; $k++) {
                    //Choises
                    ...
                    mysqli_query($conn, " insert into `poollangchoices` (`Choise`,`IdLang`,`idpc`, `Idpt`) 
                        values('" . $Choise . "','" . $IdLang . "','" . $idpc . "','" . $Idpt . "') ; ");
                }//END FOR
                $Rows = mysqli_fetch_assoc($Recordset);
            }//END FOR
        }//END IF
                                ...
                                <script language="javascript" type="text/javascript">
                                    document.onkeydown = document.onkeypress = function (evt) {
                                        ...
                                        if (keyCode) {
                                            if (evt.ctrlKey) {
                                                if(keyCode==83){
                                                                        document.getElementById("SaveEditPool").click();
                                                    return false;
                                                }
                                                return false;
                                            }
        ...
        $EditPool = '<script src="Blocks/Pool/SpryValidationTextField.js" type="text/javascript"></script>
					...
					' . (PoolInterval) . ' , ' . (From) . ' : 
					  ...
					  <span class="textfieldRequiredMsg">' . (Avalueisrequired) . '</span></span>
                                          <br/><input onclick="NoStartOrEndDate();" name="NoDateInterval" id="NoDateInterval" type="checkbox"  />' . NoDateInterval . '
					  <br/>
					    ' . (ReadyToPublish) . ' ?  
					      <select class="select" name="published" id="published">
					        ' . $selpublished . '
        ...
        for ($k = 0; $k < $NmbrOfCheked; $k++) {
			...
			where `Idpt`='" . $Idpt . "'  order by `idpc`;";

            $db_choises = new db();
            $choises = $db_choises->get_results($query);
            if ($choises) {
                $j = 0;
        ...
        $EditPool .= '</table>
					<input class="submit" name="SaveEditPool" id="SaveEditPool" type="submit" value="' . (save) . '">
						</form>
						<script type="text/javascript">
						<!--
						';
        ...
        $EditPool .= '//-->\n
					...
					onUpdate       :    catcalc
                            ...
                            <script type="text/javascript">
                            function NoStartOrEndDate(){
                                if(document.forms["formEditPool"].NoDateInterval.checked){
                                    document.forms["formEditPool"].poolstart.value="0000-00-00 00:00:00";
                                    document.forms["formEditPool"].poolend.value="0000-00-00 00:00:00";
                                    document.forms["formEditPool"].poolstart.disabled =true;
                                    document.forms["formEditPool"].poolend.disabled =true;
                                }
                                else{
                                    document.forms["formEditPool"].poolstart.value="";
                                    document.forms["formEditPool"].poolend.value="";
                                    document.forms["formEditPool"].poolstart.disabled =false;
                                    document.forms["formEditPool"].poolend.disabled =false;
                                }
