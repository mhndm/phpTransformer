...
global $dbUserPass, $dbBaseName, $dbHostName, $DisplayErrors;

define("EZSQL_DB_USER", $dbUserPass[0][0]);   // <-- mysql db user
define("EZSQL_DB_PASSWORD", $dbUserPass[0][1]);  // <-- mysql db password
define("EZSQL_DB_NAME", $dbBaseName);  // <-- mysql db pname
define("EZSQL_DB_HOST", $dbHostName); // <-- mysql server host
define("DisplaySQLErrors", $DisplayErrors);
// ==================================================================
//	ezSQL Constants
...
class db {
    ...
    function db($dbuser = EZSQL_DB_USER, $dbpassword = EZSQL_DB_PASSWORD, $dbname = EZSQL_DB_NAME, $dbhost = EZSQL_DB_HOST) {
    ...
    function select($db) {
        if (!@mysql_select_db($db, $this->dbh)) {
            $this->print_error("<ol><b>Error selecting database <u></u>!</b><li>Are you sure it exists?<li>Are you sure there is a valid database connection?</ol>");
            die('no db');
        }
        /* modified by mhndm */
    ...
    function get_var($query = null, $x = 0, $y = 0) {
        ...
        if (isset($values)) {
            if ($values[$x] != null) {
                return $this->stripslashes_deep($values[$x]);
            } else {
                return null;
            }
    ...
    function get_col($query = null, $x = 0) {
        ...
        return $this->stripslashes_deep($new_array);
    }
    ...
    function stripslashes_deep($value) {
        $value = is_array($value) ? array_map('db::stripslashes_deep', $value) :stripslashes($value);
