...
function audit_news($news_infos=array("t" => "","i" => "","u" => "","l_i" => ""))
{
    ...
    if(!$is_valid_token_var)
    ...
    else
        {
            ...
            $last_news_row = $db->get_row("select nl.Tilte as news_title,n.IdNews as news_id from languages as l,news as n,newslang as nl where l.IdLang = nl.IdLang and n.IdNews = nl.IdNews and n.Active != 1 and n.Deleted != 1 and l.LangName = 'Arabic' and n.Date >= '$last_news_date' and n.IdNews != '$last_news_id' Order By n.`Date` asc");
            if($last_news_row)
                {
...
function contact_us($email_infos = array("c" => "","d"=>"","e"=>"","f"=>"","l" => "","m"=>"","r" => "","t"=>""))
    {
        ...
        $title = isset($email_infos['ti']) && !empty($email_infos['ti'])?InputFilter($email_infos['ti'])." ".$email : "رسالة من تطبيق سايبر أمان"." ".$email;
...
function ping_news($news_id)
    {
        ...
        ob_start();
        $title = $db->get_var("select nl.Tilte from newslang,languages where l.IdLang = nl.IdLang and l.LangName = 'Arabic' and nl.IdNews = '$news_id'");
        ...
        if($android_notification_id)
            {
                ...
                $data_message = api_androidprepare_message('1', "false", str_replace(array('&nbsp;'), array(' '), strip_tags($message)), "رسالة من إدارة سايبر أمان", $message, '1',array("m" => 1));
                $response = api_androidsendNotification($android_key, $android_id, $data_message);
