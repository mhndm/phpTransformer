...
function parse_request()
    {
        global $method,$default_language;
...
function sign_up($user_infos = array("n" => "","p" => "","g" => "","c" => "","f" => "","e" => "","ph" => "","u" => "","a" => "","ap" => ""))
{
    ...
    $is_already_registered_with_notification = $db->get_var("select Id from app_visitors where uuid = '$uuid'");
    if($is_already_registered_with_notification)
        {
            $db->query("delete from app_visitors where Id = '$is_already_registered_with_notification'");
        }
    ...
    if(is_object($already_user_row))
        {
            ...
            $db->query("delete from app_visitors where uuid = '$passed_device_id'");
...
function send_news($news_infos = array("t" => "","l" => "","c" => "","a" => "","ca" => "","i" => "","v" => "","u" => "","ti" => "","lo" => "","uu" => ""))
{
    ...
    if(isset($news_infos['t']))
    ...
    else
        {
            ...
            $news_agency = "Guest";
            $news_active = 0;
            $uuid = isset($news_infos['uu']) && $db->get_var("select uuid from app_visitors where uuid = '".InputFilter($news_infos['uu'])."'")?InputFilter($news_infos['uu']) : "";
        }
    ...
    $insert_news_sql = "insert into news(IdNews,IdUserName,Date,Active,Hits,NewsPic,Deleted,urgent,agency,location,uuid) values ('$news_id','$user_id','$news_date','$news_active','$news_hits','$news_image','$news_deleted','$news_urgent','$news_agency','$news_location','$uuid')";
...
function set_user_google_registration_id($user_infos = array('t' => '','id' => '','ty' => 'a','u' => ''))
    {
        ...
        if(isset($user_infos['t']))
        ...
        else if(isset($user_infos['u']))
            {
                ...
                $is_valid_uuid_var = $db->get_var("select uuid from app_visitors where uuid = '$passed_uuid'");
                $passed_notifications_registration_id = $user_infos['id'];
                if($is_valid_uuid_var)
                    {
                        if($user_infos['ty'] == 'ap')
                            $notifications_query = "update app_visitors set apple_id = '$passed_notifications_registration_id' where uuid = '$is_valid_uuid_var'";
                        else
                            $notifications_query = "update app_visitors set android_id = '$passed_notifications_registration_id' where uuid = '$is_valid_uuid_var'";
                    }
                else
                    {
                        if($user_infos['ty'] == 'ap')
                            $notifications_query = "insert into app_visitors(android_id,apple_id,uuid)values('','$passed_notifications_registration_id','$passed_uuid')";
                        else
                            $notifications_query = "insert into app_visitors(android_id,apple_id,uuid)values('$passed_notifications_registration_id','','$passed_uuid')";
                    }
                $db->query($notifications_query);
                ...
                foreach($last_reporters_res as $reporter_row)
                    {
                        ...
                        $result["r"][$index] = array("f" => $reporter_row->user_full_name,"i" => $reporter_row->user_id,"n" => $reporter_row->user_nickname,"p" => $reporter_pic,"pt" => $reporter_row->user_points);
                        $index++;
                    }
            }
        else
        ...
        if(isset($notification_infos['t']) && isset($_SESSION['notification'][$notification_infos['t']]['token']))
            {
                ...
                if($type == 2)//notify admins
                ...
                else
                    {
                        ...
                        if($is_urgent === 1)
                            $android_guest_count_var = $db->get_var("select count(*) from app_visitors where android_id <> '0' and android_id <> ''");
                                        ...
                                        if($is_urgent === 1)
                                            {
                                                $paginated_users = $db->get_results("select UserId,android_id,apple_id,uuid,PrefLang from users where android_id <> '0' and android_id <> '' 
                                                                                        UNION
                                                                                    select id,android_id,apple_id,uuid,PrefLang from app_visitors where android_id <> '0' and android_id <> '' limit $start,500");
                                            }
                                        else
                                            {
                        ...
                        if($is_urgent === 1)
                            $apple_guest_count_var = $db->get_var("select count(*) from app_visitors where apple_id <> '0' and apple_id <> ''");
                                ...
                                if($is_urgent === 1)
                                    {
                                        $paginated_users = $db->get_results("select UserId,android_id,apple_id,uuid,PrefLang from users where apple_id <> '0' and apple_id <> '' 
                                                                                        UNION
                                                                                    select id,android_id,apple_id,uuid,PrefLang from app_visitors where apple_id <> '0' and apple_id <> ''");
                                    }
                                else
                                    {
                                        $paginated_users = $db->get_results("select UserId,android_id,apple_id,uuid,PrefLang from users where apple_id <> '0' and apple_id <> ''");
                                ...
                                if($member->PrefLang == $language->LangName)
                                    {
                                        ...
                                        $is_guest_var = $db->get_var("select Id from app_visitors where Id = '".$member->UserId."'");
                                        $user_type = $is_user_var ? "users" : "app_visitors";
                        ...
                        if($user_row->PrefLang == $language->LangName)
                            {
                                ...
                                $is_guest_var = $db->get_var("select Id from app_visitors where Id = '".$user_row->UserId."'");
                                if(isset($note_data['e']))
                                    {
                                        $send = 1;
                                    }
...
function send_private_notification($notification_infos = array("t" => "","u" => "","m"))
    {
        ...
        if(!isset($notification_infos['u']) || (!$db->get_var("select uuid from app_visitors where uuid = '".InputFilter($notification_infos['u'])."'") && !$db->get_var("select uuid from users where uuid = '".InputFilter($notification_infos['u'])."'")))
            {
                return json_encode(array("s" => '003'));
            }
        ...
        if(!$android_notification_id)
            {
                $android_notification_id = $db->get_var("select android_id from app_visitors where uuid = '$uuid'");
                if(!$android_notification_id)
                    {
                        $apple_notification_id = $db->get_var("select apple_id from users where uuid = '$uuid'");
                        if(!$apple_notification_id)
                            {
                                $apple_notification_id = $db->get_var("select apple_id from app_visitors where uuid = '$uuid'");
                            }
                    }
            }
        if($android_notification_id)
