...
class UploadHandler {
    ...
    protected function create_scaled_image($file_name, $version, $options) {
        ...
        if (is_file('../../../img/water.png')) {
            include_once '../../../../watermark.php';
            Watermark($file_path, '../../../img/water.png', $file_path);
        }
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $path = strpos($file_path, 'uploads/gallery/Albums');
    ...
    public function delete($print_response = true) {
        ...
        $file_path = substr($file, strpos($file, 'uploads/gallery/Albums'));
        $file_path = str_replace('/thumbs', '', $file_path);
        $db_del = new db();
        $IdMedia = $db_del->get_var(" select `IdMedia` from `gallery` where `Path`='" . $file_path . "' ;");
        $db_del->query(" delete from `gallerylang` where `IdMedia`='" . $IdMedia . "' ; ");
        $db_del->query(" delete from `gallery` where `IdMedia`='" . $IdMedia . "' ; ");
    ...
    public function add_file_to_db($Path) {
        ...
        if ($Path) {
            ...
            $db = new db();
            $InDatabase = $db->get_var("SELECT `IdMedia` FROM `gallery` where `Path`='" . $Path . "';");
			...
			VALUES ('" . $IdMedia . "', '" . $Path . "', '" . $AddDate . "', '', '', '" . $MediaType . "', '1');");
                ...
                if ($Langs) {
                    foreach ($Langs as $language) {
                        ...
                        $dbInsetLang = new db();
                        //$caption = substr($Path, strrpos($Path, "/") + 1, (strrpos($Path, ".") - (strrpos($Path, "/") + 1)));
                        $db->query("INSERT INTO `gallerylang` ( `IdMedia` , `IdLang` , `Caption` , `Desc` , `Place` , `Tags` )
                                    VALUES ('" . $IdMedia . "', '" . $IdLang . "', '" . $this->original_file_name . "', '', '', '')");
                    }//end foreach
                }//end if
            }//end if
        }//end if
