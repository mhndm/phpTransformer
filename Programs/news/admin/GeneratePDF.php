...
if(isset($_GET['idnews'])) {
    ...
    if ($TotalRecords>0) {
        ...
        if ($TotalRecords>0) {
            ...
            list($width, $height, $type, $attr) = getimagesize('../images/'.$NewsPic);
            $NewsImg='<img '.$attr .' style="padding: 5px 5px 5px 5px;margin: 3px 2px 1px 2px;display: inline;" alt="" src="../../../Programs/news/images/'.$NewsPic.'"/>';
            // directory location up to steps
            $Breif = str_replace('src="', 'src="../../../', $Breif);
            $Breif = str_replace("src='", "src='../../../", $Breif);
            $Breif = str_replace('SRC="', 'SRC="../../../', $Breif);
            $Breif = str_replace("SRC='", "SRC='../../../", $Breif);
            ...
            $FullMessage = str_replace('SRC="', 'SRC="../../../', $FullMessage);
            $FullMessage = str_replace("SRC='", "SRC='../../../", $FullMessage);
            $Breif = str_replace('src="../../../http', 'src="http', $Breif);
            $Breif = str_replace("src='../../../http", "src='http", $Breif);
            $Breif = str_replace('SRC="../../../HTTP', 'SRC="HTTP', $Breif);
            $Breif = str_replace("SRC='../../../HTTP", "SRC='HTTP", $Breif);
            ...
            // add http to internal links
            $Breif = str_replace('href="', 'href="'.$WebsiteUrl, $Breif);
            $Breif = str_replace("href='", "href='".$WebsiteUrl, $Breif);
            $Breif = str_replace('HREF="', 'HREF="'.$WebsiteUrl, $Breif);
            $Breif = str_replace("HREF='", "HREF='".$WebsiteUrl, $Breif);
            $Breif = str_replace('href="'.$WebsiteUrl.'http','http', $Breif);
            $Breif = str_replace("href='".$WebsiteUrl.'http', 'http', $Breif);
            $Breif = str_replace('HREF="'.$WebsiteUrl.'HTTP','HTTP', $Breif);
            $Breif = str_replace("HREF='".$WebsiteUrl.'HTTP', 'HTTP', $Breif);

            $pdfPage  = $Date .'<br/>'
                    .$NewsImg . $Breif .  $FullMessage.  $Note. '<p><strong>'
            ...
            $pdf->SetHeaderData('pdf_logo.gif', PDF_HEADER_LOGO_WIDTH, $Tilte, $SubTitle);
            // set header and footer fonts
            $pdf->setHeaderFont(Array('al-arabeh', '', '16'));
            $pdf->setFooterFont(Array('al-arabeh', '', PDF_FONT_SIZE_DATA));
            //set margins
            ...
            $pdf->Output("../../../downloads/news/pdf/news-".$IdNews."-".$Lang.".pdf", "F"); // Save PDF to a local file
