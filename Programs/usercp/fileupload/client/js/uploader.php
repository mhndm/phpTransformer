...
if(isset($_GET['up'])){
    $up= strip_tags($_GET['up']);
}else{
    $up='Upload a file';
}

if(isset($_GET['IdMedia'])){
    $IdMedia = strip_tags($_GET['IdMedia']);
}else{
    $IdMedia ='/';
}
?>
...
qq.extend(qq.FineUploader.prototype, {
    ...
    _onComplete: function(id, name, result, xhr){
        ...
        if (result.success) { 
            var file_name = this._handler.getName(id);
            var extfile = file_name.split('.').pop();
            extfile = extfile.toLowerCase();
            
            $("#MiniNewsPic").html('<img style="max-width:200px; max-height:200px;" src="'+'<?php echo $IdMedia .'/avatar_128.' ?>'+extfile+'?v=nacache" alt="" />');
            $("#UserPic").html('<img style="max-width:200px; max-height:200px;" src="'+'<?php echo $IdMedia .'/avatar_128.' ?>'+extfile+'?v=nacache" alt="" />');
