...
class UploadHandler {
    ...
    function __construct($path, $url, $WebsiteUrl, $options = null, $initialize = true) {

        $this->options = array(
            'WebsiteUrl' => $WebsiteUrl,
            'script_url' => $this->get_full_url($url) . '/',
            ...
            'inline_file_types' => '/\.(gif|jpe?g|png|wmv|flv|swf|rm|avi|mp4|mpeg|mpg|youtube|mp3|wma|ra|pdf|ppt|pptx|tiff|doc|docx|xls|xlsx|rtf|txt|ods|odt|odp|odg)$/i',
            // Defines which files (based on their names) are accepted for upload:
            'accept_file_types' => '/\.(gif|jpe?g|png|wmv|flv|swf|rm|avi|mp4|mpeg|mpg|youtube|mp3|wma|ra|pdf|ppt|pptx|tiff|doc|docx|xls|xlsx|rtf|txt|ods|odt|odp|odg)$/i',
    ...
    protected function get_full_url($url) { //modified by mhndm
        ...
        return
                ...
                (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : ($_SERVER['SERVER_NAME'] .
                        ...
                        $_SERVER['SERVER_PORT'] === 80 ? '' : ':' . $_SERVER['SERVER_PORT']))) . $url;
    }
    ...
    protected function get_file_objects($iteration_method = 'get_file_object') {
        ...
        return array_values(array_filter(array_map(
                                        array($this, $iteration_method), array_diff(scandir($upload_dir, 0), array('index.htm', 'index.html'))
                                )));
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null) {
        ...
        $this->original_file_name = substr($file->name, 0, strrpos($file->name, '.'));
        $file_ext = explode('.', $file->name);
        $file->name = substr(md5($file->name . date('Y-m-d H:i:s')), 0, 10) . '.' . end($file_ext);
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
                    ...
                    $path = strpos($file_path, 'Themes/Default/slider');
                    $path = substr($file_path, $path);
                    if (!strpos($path, 'index.htm')) {
                        $this->add_file_to_db($path);
                    }
                }
            } else {
    ...
    public function delete($print_response = true) {
        ...
        $file_path = substr($file, strpos($file, 'Themes/Default/slider'));
        $file_path = str_replace('/thumbs', '', $file_path);
        $db_del = new db();
        $IdMedia = $db_del->get_var(" select `id` from `thm_default_slider` where `path`='" . $file_path . "' ;");
        $db_del->query(" delete from `thm_default_caption` where `id`='" . $IdMedia . "' ; ");
        $db_del->query(" delete from `thm_default_slider` where `id`='" . $IdMedia . "' ; ");
        return $this->generate_response(array('success' => $success), $print_response);
    }

    /* added by mhndm */

    public function add_file_to_db($Path) {

        if ($Path) {
            $db = new db();
            $InDatabase = $db->get_var("SELECT `id` FROM `thm_default_slider` where `path`='" . $Path . "';");
            if ($InDatabase) {//cheking if this path aleady in the db
                return false;
            } else {
                //$Path = InputFilter($_GET['galid']);
                $id = GenerateID('thm_default_slider', 'id');
                $AddDate = date('Y-m-d H:i:s');
                $MediaType = $this->media_file_type($Path);

                $db->query("INSERT INTO `thm_default_slider` (`id`, `path`, `href`) VALUES ('" . $id . "', '" . $Path . "', '');");
                ...
                if ($Langs) {
                    foreach ($Langs as $language) {
                        ...
                        $db->query("INSERT INTO `thm_default_caption` (`id`, `id_lang`, `caption`) 
                                            VALUES ('" . $id . "', '" . $IdLang . "', '');");
                    }//end foreach
                }//end if
            }//end if
    ...
    public function media_file_type($Path) {
        if ($Path) {
            if (is_dir($Path)) {
                return 'folder';
            } else {
